...
createApp({
    data() {
        return {
            runtimeInfo: jsBridgeApi.runtimeInfo,
    ...
    methods: {
        ...
        onBtnOpenShareClick() {
            jsBridgeApi.openShare('原神三周年', '登录就送 999 原石\nhttps://ys.mihoyo.com')
        },
        onBtnCheckPermissionClick(permission) {
            let isGranted = jsBridgeApi.checkPermission(permission)
            jsBridgeApi.showToast(permission + ': ' + isGranted)
        },
    },
}).mount('#app')
